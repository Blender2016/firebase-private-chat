...
const onRegisterStart = () => {
  return {type: actionTypes.ON_REGISTER_START}
}

const onRegisterSuccess = (id, imageUrl, userName, email, token, isOnline, isAdmin, loggedOutAt) => {
  return {
...
const onRegisterFail = () => {
  return {type: actionTypes.ON_REGISTER_FAIL}
}

const isRegistered = () => {
  return {type: actionTypes.IS_REGISTERED}
}
  ...
  return dispatch => {
    ...
    axios.post('/register', userData).then(res => {
      if (res.data.isRegistered) {
        ...
        dispatch(onRegisterSuccess(id, imageUrl, userName, email, token, isOnline, isAdmin, loggedOutAt))
        history.push('/chat')
