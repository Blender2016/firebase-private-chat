...
const onUpdateStart = () => {
  return {type: actionTypes.ON_UPDATE_START}
}
const onUpdateFail = () => {
  return {type: actionTypes.ON_UPDATE_FAIL}
}
const onUpdateSuccess = (userName, userImage) => {
  return {type: actionTypes.ON_UPDATE_SUCCESS, userName: userName, userImage: userImage}
}
...
export const onProfileUpdate = (newData) => {
  ...
  return dispatch => {
    ...
    axios.patch('/updateprofile', newData).then(res => {
      ...
      firebaseApp.database().ref('/users/' + id).update({name: username, imageUrl: userImage})
      dispatch(onUpdateSuccess(username, userImage))
    }).catch(err => {
      console.log('userlogout failed', err)
