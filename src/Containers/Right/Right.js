...
class Right extends Component{
    ...
    render(){
        ...
        var adminItem = ( <Menu.Item>
            <Link to="/admin"><Icon type="dashboard" style={{marginRight:'5px'}}/> Admin Page</Link>
         </Menu.Item> );

         var admin = this.props.isAdmin ? adminItem : null ;
        ...
        //   console.log('tohamy----->',this.props.notifications.map(notification=>notification.id));
          ...
          let notificationItems = this.props.notifications.map(notification=>(
                    notification.otherId === this.props.ownerId ?
                        <Menu.Item key={uuidv4()}>
                            <a href="#"><Avatar  src={this.getUserImage(notification.ownerId)} style={{marginRight:'5px', height:'15px',width:'15px'}}/>
                                 <span style={{fontWeight:'bold',color:'#bae637'}}>{this.getUserName(notification.ownerId)}</span>
                                 <span style={{color:'#1890ff', marginLeft:'5px'}}>send you a message </span>
                             </a>
                        </Menu.Item>
                        : null
                     ));
          ...
          const allNotifications = (
            <Menu>
                 {notificationItems}
                 <Menu.Item >
                    <a href="#" style={{textAlign:'center'}} onClick={(e)=>this.props.removeAllNotification(e)}>
                        <Icon type="delete" style={{marginLeft:'auto'}}/>
                    </a>
                </Menu.Item>
            </Menu>
          );
        ...
        return(
            <div className={Styles.Right}>
                ...
                <span>To: <span className={Styles.Name}>
                   <Avatar src={this.props.otherUserImage} style={{width:'25px',height:'25px'}}/>
                {this.props.otherName}</span></span>

                <Dropdown overlay={allNotifications} >
                    <a className="ant-dropdown-link" href="#" style={{marginLeft: '160px'}}>
                    <Badge count={this.getNumOfNotifications()} style={{backgroundColor:'#bae637'}}>
                ...
                <Dropdown overlay={menu} >
                    <a className="ant-dropdown-link" href="#" style={{float: 'right'}}>
                     <Avatar src={this.props.userImage} style={{width:'25px',height:'25px'}}/>
